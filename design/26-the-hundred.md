...
1. **`E!` Amendments that are read.** Every bill carries an amendments field
...
2. **`E!` Supply.** `confidenceSupply` is a promise to vote through a budget
...
3. **`E` Guillotine and closure.** A government that cannot win on time buys
   ...
   available.
4. **`E` The casting vote.** A tie in a chamber of 280 with an absolute
   majority rule is possible and currently unhandled.
5. **`C` Private members' bills.** A slot the government does not control, one
   per session, drawn by ballot among backbenchers.
6. **`E` Committee stage as a real stage.** A bill in committee is currently a
   bill waiting. Give it a composition, a chair and the power to report
   amended.
7. **`E` Points of order.** A one-click procedural interruption that costs
   nothing and achieves nothing, available always. A parliament without
   pointless procedure does not read as one.
8. **`B` The second chamber.** The bible has no revising body. Deciding there
   isn't one is as good as inventing one, but it should be decided.
9. **`C` Urgent questions.** An opposition instrument that forces a minister to
   the despatch box on the day, consuming order-paper time the government
   wanted.
10. **`E` Divisions that can be challenged.** A voice vote taken, then
...
11. **`E!£` The named backbencher.** The whip currently moves a number of
...
12. **`C` Currents with leaders.** A faction whose spokesman can be bought,
    promoted or destroyed separately from the party.
13. **`E` Defection.** A member who crosses is currently a `cross` effect on a
    count. A member who crosses and is *named* is a news story for three
    sittings.
14. **`E` The letter.** Signatures already exist (`signaturesAtLeast`). Let the
    player see who signed, and let a signature be withdrawn at a price.
15. **`C` A party that is not a party.** A registered independents' group with
    a convenor and no whip — `ind` holds six seats and behaves like a bloc.
16. **`E` The payroll vote.** Ministers and their aides cannot rebel. Counting
    them separately makes every reshuffle a vote calculation.
17. **`C` Deselection.** A local party that can sack its own member between
    elections, which is a threat the leadership does not control.
18. **`E` Loyalty per member, not per party.** The deep version of 11; probably
    too deep for §7.6, listed so the line can be drawn deliberately.
19. **`C` The awkward squad.** Four or five members who vote against on
    principle, reliably, and whose support therefore means something.
20. **`E` A whip who can fail.** `payWhips` always works. A whip operation that
    can be refused, publicly, is a much better scene.

## III. MONEY, LOBBYING, INTERESTS

21. **`E!` The actor store.** `design/24` A1. Bodies outside the chamber with
    resources, patience and an agenda. The prerequisite for 22–28.
22. **`E!` Lobbying.** `design/24` A2. Substrate neutrality is currently
    unreachable by any means the player controls; lobbying is the mechanism
    that makes an ending purchasable at a cost.
23. **`C` The register of interests.** Who each member is paid by, published,
    and readable by the player before a division.
24. **`E` Donations with a memory.** Money taken in session one that is an
    embarrassment in session three.
25. **`E` The revolving door.** A minister who leaves for the body they
    regulated. A slow, quiet scandal generator that needs no new verb.
26. **`C` Trade unions as actors.** `content/labour.js` already models the
    workforce and nothing represents it politically.
27. **`E` Procurement.** A contract is the cheapest way to move a station's
    opinion and the easiest thing to be caught doing.
28. **`B` Whether the player can be personally corrupt.** The bible is silent.
    It changes the tone of the whole game and should be answered on purpose.
29. **`C` The think tank.** An actor that produces *numbers* rather than votes,
    and whose numbers the player may cite or dispute.
30. **`E` Capital that decays.** The ledger (`setup.capital`) never rots. A
    favour owed forever is not a favour.

## IV. THE PUBLIC, MEDIA, OPINION

31. **`E!` The pollster.** `design/24` A3. `reported()` already applies
    per-party error; generalising it is most of a polling system.
32. **`E` Polls that can be wrong in a direction.** A house effect per pollster
    is one field and turns a number into a source.
33. **`C` The press, with mastheads.** A wire item attributed to a paper with a
    known lean reads completely differently from an unattributed one.
34. **`E` A leak.** State the player can see arriving in the press before they
    have acted on it.
35. **`C` The interview.** An event type where the choice is a *form of words*
    and the effect is what gets quoted.
36. **`E` Salience, not just position.** The public may agree with the player
    and not care. Salience is the field that makes a popular policy losable.
37. **`C` Public opinion that is regional.** Station-level opinion exists in
    the model and is barely surfaced.
38. **`E` The by-election as a poll.** One seat, real result, disproportionate
    meaning.
39. **`C` Vox pops.** Three lines from a named nobody on a station, after a
    division. Cheap, and the only place an ordinary person speaks.
40. **`E` A scandal clock.** A story that decays unless fed, so the player's
    choice is whether to feed it.

## V. ELECTIONS AND THE FRANCHISE

41. **`E!` The parliament ends.** Three sessions, then dissolution. There is
    currently no cap: a run that does not settle runs forever, and I measured
    190 empty sittings.
42. **`E` The campaign as a chapter.** `design/10` specifies it; nothing
    implements it.
43. **`E` Manifesto commitments.** An undertaking made to the electorate rather
    than to a party, tested at the next election rather than by a date.
44. **`C` The franchise itself as a bill.** Who counts as a person is the
    game's subject; who counts as a *voter* is barely touched.
45. **`E` Boundary review.** `apportionment` already exists. A commission that
    redraws seats between parliaments is an enormous lever nobody is pulling.
46. **`E` Electoral system change.** `district_divisor` and `list_divisor` are
    already fields and already tunable by a bill. Almost free.
47. **`C` Turnout.** A number the player can move and which moves results in a
    direction they may not like.
48. **`E` The threshold as a weapon.** `threshold_pct` is 4. A bill that moves
    it to 6 kills two parties and is entirely legal.
49. **`C` Coalition negotiation after the election.** The president has a
    `formation` power that has never been exercised.
50. **`E` Losing the election and continuing.** Opposition is a game state the
    engine cannot currently represent.

## VI. THE EXECUTIVE, THE PRESIDENT, THE STATE

51. **`E` The reshuffle as a scene.** `cabinet` and `vacate` exist; appointing
    is a fill-a-post click with no politics in it.
52. **`E!` Collective responsibility.** A minister who disagrees must resign or
    lie. This is the executive/legislature split the Chamber tab now makes
    visible, given teeth.
53. **`C` The civil service.** An institution that has views, tenure, and
    advice the player may reject at a cost.
54. **`E` The president's referral power.** Listed in `setup.president.powers`
    and never used.
55. **`E` Dissolution as a threat.** The president holds it; the player should
    be able to fear it.
56. **`C` The permanent secretary's note.** A document that tells the player
    what the department thinks, which is not what the minister thinks.
57. **`E` Emergency powers.** `shed_order_authority` is already a law field
    pointing at the engineering authority. That is a constitutional crisis
    waiting in a config line.
58. **`E` Judicial review.** An Act struck down after passing is the strongest
    possible argument for drafting carefully.
59. **`C` The inquiry.** Established under pressure, reports two sessions
    later, and by then the question has changed.
60. **`E` Confidence as a motion, not only a count.** The opposition tabling it
    is a scene; a scalar dropping below a line is not.

## VII. STATIONS, FEDERALISM, FOREIGN

61. **`C` Station governments with names.** Thirty stations, no local
    politicians.
62. **`E` A station that legislates.** The federal settlement is an *ending*;
    federalism should be a live tension before then.
63. **`E` Internal migration.** `federal_fudge`'s own summary says the cost is
    paid in migration, which the engine does not model.
64. **`C` The station that secedes.** Or threatens to, credibly, once.
65. **`E` Thermal as politics.** `thermal_margin` is a loss condition and
    nothing else. Heat is the setting's scarcest resource and its least
    political one.
66. **`E` Trade with somewhere.** `design/11` exists; foreign affairs does not.
67. **`C` The anchorage concession.** `anchor_kepler` is a bill at assent about
    a treaty nobody has met.
68. **`E` Supply chains that can be cut.** `consumables` at 71 and falling is a
    crisis the game cannot currently stage.
69. **`C` The capital.** The Winter Garden exists, has one non-voting seat, and
    has never appeared in a sentence.
70. **`E` Station closure as a decision, not a defeat.** Closing one is
...
71. **`E!` Subjective time as a resource.** A legislator running fast reads
...
72. **`C` The proxy vote for an instance.** Who votes when a member is forked
    is a standing-orders question, not a metaphysical one.
73. **`E` Continuity of membership.** A member who dies and is restored — are
    they the same member, and does their seat fall vacant? A by-election
    hinges on an ontological answer.
74. **`C` The register of persons.** `continuity_registration` is a drafting
    bill about exactly this and has no content around it.
75. **`E` Quorum with instances.** Forty members and two hundred instances.
76. **`E` Pairing across substrates.** Pairing landed this week; a pair between
    an embodied member and an instance is where the courtesy breaks.
77. **`C` The oath.** What a new member swears to, and who cannot swear it.
78. **`B` Whether an instance can hold office.** Not whether it is a person —
    whether it can be Treasurer.
79. **`E` Time-limited terms in subjective years.** The bible names
    subjective-time term limits; nothing implements them.
80. **`C` Committee work done by fast staff.** Named in the bible as a quiet
    practice. A scandal in waiting, and a procedural one.

## IX. INTERFACE, FEEDBACK, LEGIBILITY

81. **`E!` The vote held until dismissed.** Owed from the author's 14 September
    list: the diagram stays as a record of the division until the caption is
    closed.
82. **`E!` Abstention and absence, shown.** The three-way count landed in the
    engine on the 14th and the UI still renders two.
83. **`E` The pairing UI.** Engine landed 15 September, no control exists.
84. **`E` A bill's history.** Every stage, every division, every amendment, on
    one page.
85. **`C` The letterhead.** The bill dossier headed as an in-world instrument.
    Needs a slot in `js/artifacts.js` first.
86. **`E` The diff.** After an event, what changed, in two lines. `acted()`
    already reports moves; generalise it.
87. **`E` A search.** Fifty-four characters, 141 seats, thirty stations and no
    way to find one.
88. **`E` The clock, everywhere.** `dayLine()` shows the day's divisions in one
    place; sittings-until-rise belongs in the topbar.
89. **`C` Tips that teach procedure.** `js/tips.js` explains the terminal.
    Nothing explains what a second reading is *for*.
90. **`E` Undo, once.** §12.13 gave every action a report; a report the player
    can act on is better than one they can only read.

## X. RUN SHAPE AND REPLAY

91. **`E!` The settlement floor.** Measured this week: a settlement fires at
    sitting 7 on one play policy. An ending must be *carried*, not reached.
92. **`E` Events that fire more than once.** Twenty of twenty-four are `once`,
    which is why the pool goes dry at sitting 9.
93. **`B` What a second run is for.** Four endings is the current answer.
    `design/14` §6's second leader is a better one.
94. **`C` A different opening.** Same parliament, different portfolio.
95. **`E` Difficulty as arithmetic, not as multipliers.** A tighter majority is
    a harder game and needs no difficulty setting.
96. **`E` The record between runs.** What the last Commonwealth settled on,
    shown at the start of the next.
97. **`C` An epilogue per station.** Where the settlement landed, locally.
98. **`E` A run that can be abandoned honourably.** Resignation as an ending
    rather than a quit.
99. **`B` Whether the player can be wrong.** The author rejected the
    right-once/wrong-once rule as too reductive; the question it was answering
    is still open.
100. **`E` A named seed.** Determinism is already total (§1.5). Surfacing the
